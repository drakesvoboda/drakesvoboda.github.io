...
const interpolate = (low, high, percentage) =>
  high * percentage + low * (1 - percentage);
...
function NoiseSphere(element) {
  ...
  const scrollHandler = (event) => {
    ...
    if (y >= scrollDist) {
      ...
      camera.position.x = interpolate(
        ...cameraPositionRange.x,
        ease(scrollPercentage)
      );
      camera.position.y = interpolate(
        ...cameraPositionRange.y,
        ease(scrollPercentage)
      );
      camera.position.z = interpolate(
        ...cameraPositionRange.z,
        ease(scrollPercentage)
      );

      renderer.domElement.style.opacity = interpolate(
        ...opacityRange,
        ease(scrollPercentage)
      );
  ...
  const speedRange = [0.5, 0.75];

  const scale = new Inertia(scaleRange[0], 0.1);
  const speed = new Inertia(speedRange[0], 0.1);
